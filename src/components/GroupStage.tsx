...
function calcMatchPoints(pick: MatchPick, match: Match): number | null {
  ...
  const exactHome = pick.teamHomeGoals === match.teamHomeScore;
  const exactAway = pick.teamAwayGoals === match.teamAwayScore;

  if (exactHome && exactAway) return 3;

  const pickWinner = pick.teamHomeGoals > pick.teamAwayGoals ? 'home'
    : pick.teamHomeGoals < pick.teamAwayGoals ? 'away' : 'draw';
  const realWinner = match.teamHomeScore > match.teamAwayScore ? 'home'
    : match.teamHomeScore < match.teamAwayScore ? 'away' : 'draw';
...
export default function GroupStage({
  ...
  const filteredMatches = groupMatches.filter((m) => {
    ...
    const matchesSearch =
      homeName.includes(q) || awayName.includes(q) || venue.includes(q) || city.includes(q) ||
  ...
  const handleInputChange = (matchId: number, side: 'home' | 'away', val: string) => {
    ...
    if (side === 'home') onChangePick(matchId, numeric, current.teamAwayGoals);
    else onChangePick(matchId, current.teamHomeGoals, numeric);
  };
    ...
    <div className="space-y-6" id="group-stage-predictions">
      ...
      {readOnly && (
        <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 text-indigo-900 text-sm flex items-center gap-3">
          ...
          <span>Estás visualizando las predicciones de <strong>{participantName}</strong> de la Fase de Grupos en modo lectura.</span>
        </div>
      )}

      {/* LISTADO DE GRUPOS COLAPSABLE */}
      <div className="border border-slate-100 rounded-2xl bg-white shadow-sm overflow-hidden">
        ...
        >
          ...
          {expandedGroupsList ? (
            <div className="flex items-center gap-1 text-xs text-slate-400 font-semibold"><span>Ocultar listado</span><ChevronUp className="w-4 h-4" /></div>
          ) : (
            <div className="flex items-center gap-1 text-xs text-slate-400 font-semibold"><span>Ver listado</span><ChevronDown className="w-4 h-4" /></div>
          )}
        </button>
        {expandedGroupsList && (
      ...
      {selectedGroup !== 'all' && (
        <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm">
                  ...
                  return (
                    <tr key={row.teamId} className={`hover:bg-slate-50/50 ${isTopTwo ? 'bg-emerald-50/5' : ''}`}>
                      <td className="py-2 px-4 text-center">
                        <span className={`w-5 h-5 rounded-md flex items-center justify-center font-bold text-[10px] ${idx === 0 ? 'bg-emerald-500 text-white' : idx === 1 ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-600'}`}>
                          {idx + 1}
                        </span>
                      </td>
                      ...
                      <td className="py-2 px-4 text-center font-mono font-semibold">{row.gd > 0 ? `+${row.gd}` : row.gd}</td>
                      <td className="py-2 px-4 text-center">
          ...
          <div className="text-[10px] text-slate-400 mt-2">* Los dos primeros clasifican automáticamente a la ronda de dieciseisavos (R32).</div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {filteredMatches.map((m) => {
          ...
          const points = readOnly ? calcMatchPoints(pick, m) : null;
          const pointsColor = points === 3 ? 'bg-emerald-500 text-white' : points === 1 ? 'bg-indigo-500 text-white' : points === 0 ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-400';
          ...
          return (
            ...
            >
              ...
              {/* Marcadores */}
              <div className="flex items-center justify-between gap-2 border-b border-slate-50 pb-3 relative">

                {/* Puntos obtenidos — esquina derecha arriba */}
                {readOnly && points !== null && (
                  <div className={`absolute -top-1 right-0 w-7 h-7 rounded-full flex items-center justify-center font-extrabold text-sm ${pointsColor}`}>
                    {points}
                  </div>
                )}
                ...
                <div className="flex-1 flex items-center justify-end gap-2.5 text-right">
                  <span className="font-bold text-sm text-slate-800 leading-tight hidden lg:block">{getTeamName(m.teamHomeId)}</span>
                ...
                <div className="shrink-0 flex flex-col items-center gap-1.5 px-2">
                  ...
                  {/* Texto debajo del score */}
                  {readOnly && m.completed && m.teamHomeScore !== undefined && m.teamAwayScore !== undefined ? (
                    <span className="text-[10px] tracking-widest text-slate-500 font-extrabold">
                      OFICIAL: {m.teamHomeScore} - {m.teamAwayScore}
                    </span>
                  ) : (
                    <span className="text-[10px] tracking-widest text-slate-400 font-extrabold">
                      {readOnly ? 'PREDICCIÓN' : 'INGRESA TU SCORE'}
                    </span>
                  )}
                </div>
                ...
                <div className="flex-1 flex items-center justify-start gap-2.5 text-left">
                  ...
                  <span className="font-bold text-sm text-slate-800 leading-tight hidden lg:block">{getTeamName(m.teamAwayId)}</span>
